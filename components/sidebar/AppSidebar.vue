        ...
        <VaAccordion v-model="value" multiple>
            ...
            >
                <template #header="{ value: isCollapsed }">
                    <VaSidebarItem
                        :to="route.children ? undefined : route.to"
...
export default defineComponent({
    ...
    setup: (props, { emit }) => {
        ...
        const routeHasActiveChild = (section: INavigationRoute) => {
            ...
            return section.children.some(({ to }) => route.path == to);
        };

        const setActiveExpand = () =>
            (value.value = navigationRoutes.routes.map(
                (route: INavigationRoute) => routeHasActiveChild(route),
            ));
